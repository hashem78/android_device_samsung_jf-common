...
## $DEVICE/device-proprietary-files.txt
PRODUCT_COPY_FILES += \\
EOF
...
COUNT=`expr $COUNT - $DISM`
echo >> $MAKEFILE
echo "## jf-common/proprietary-files.txt">> $MAKEFILE
if [ $COUNT != "0" ]; then
...
ifneq (\$(filter jflteatt jfltespr jfltetmo jfltevzw jfltexx jfltecan jflteusc jfltecri jfltecsp jfltezm jftddxx,\$(TARGET_DEVICE)),)
